...
Changed rake task so 

<pre><code>
rake cover_me 
</code></pre>

runs the full rspec test then launches the results correctly.  Too slow for autotest.

Added launchy >4.0.0 to launch results correctly after running.  Open was not happy.  This feels a bit hackish, but it works and avoids the:
